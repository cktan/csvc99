...
void usage(int exitcode, const char* msg)
{
    ...
    perr("    -b nbytes\n");
    perr("        split into files of at most nbytes each (at least one record)\n");
    perr("    -r nrecs\n");
...
int main(int argc, char* argv[])
{
    ...
    while ((opt = getopt(argc, argv, "hb:r:")) != -1) {
        ...
        case 'b':
            ...
            if (g_nbyte <= 0) {
                usage(1, "ERROR: invalid -b nbytes option. Please supply a +ve integer\n");
            }
            break;
        case 'r':
            g_nrec = strtol(optarg, 0, 0);
            if (g_nrec <= 0) {
                usage(1, "ERROR: invalid -r nrecs option. Please supply a +ve integer\n");
            }
            break;
        default:
